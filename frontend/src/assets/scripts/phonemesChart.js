...
function getChartWidth() {
  const rootStyling = getComputedStyle(document.querySelector('#main-chart-container'))
  return parseInt(rootStyling.width, 10)
}

function redrawChart({ dataset, ms, color, editable, initialContainerHeight, store }) {
  ...
  const chartSvg = root.append('svg')
  ...
  chartSvg
    .append("g")
    .attr("transform", `translate(${margin.left}, ${initialContainerHeight - margin.bottom})`)
    .call(xAxis)
  ...
  chartSvg
    ...
    .attr('x', 0)
    .attr('y', margin.left)
    .attr('transform', `matrix(0 -1 1 0 0 ${(initialContainerHeight - margin.top - margin.bottom) / 2})`)
  ...
  for (const y of yScale.ticks(yTickCount)) {
    const gridLineY = initialContainerHeight - margin.bottom - y * (initialContainerHeight -
      margin.top - margin.bottom) / yScale.domain()[0]
  ...
  for (const x of ms.value) {
    const gridLineX = margin.left + x * (chartWidth - margin.left - margin.right) / xScale.domain()[1]
  ...
  const chartPointContainer = chartSvg
    .append("g")

  for (const pointIndex in dataset.value) {
    const { x, y } = dataset.value[pointIndex]
    ...
    chartPointContainer
      ...
      .attr("cx", margin.left + x * (chartWidth - margin.left - margin.right) / xScale.domain()[1])
      .attr("cy", initialContainerHeight - margin.bottom - y * (initialContainerHeight - margin.top
        - margin.bottom) / yScale.domain()[0])
  }

  const tooltip = chartSvg
  ...
  function calculateChartPath() {
    chartPath.attr(
      ...
      d3.line()
        ...
        .x(({ x }) => margin.left + x * (chartWidth - margin.left - margin.right) / xScale.domain()[1])
        .y(({ y }) => initialContainerHeight - margin.bottom - y * (initialContainerHeight - margin.top
          - margin.bottom) / yScale.domain()[0])
    )
  }
  ...
  function recalculateChartPath(pointIndex, pointY) {
    const chartY = Math.round((initialContainerHeight - margin.bottom - pointY) * yScale.domain()[0] /
  ...
  function handleTooltipText() {
    ...
    const ms = Math.round(dataset.value[pointIndex]?.x)

    const x = margin.left + ms * (chartWidth - margin.left - margin.right) / xScale.domain()[1]
    const y = initialContainerHeight - margin.bottom - frequency * (initialContainerHeight - margin.top
          - margin.bottom) / yScale.domain()[0]
  ...
  function onDragEvent(event) {
    const pointIndex = parseInt(this.getAttribute("point-index"), 10)
    const pointY = Math.round(Math.max(margin.top, Math.min(initialContainerHeight - margin.bottom, event.y)))
...
export default function getChartGenerator() {
    ...
    return ({ dataset, ms, color, editable }) => {
      ...
      function onResizeHandler() {
        ...
        redrawChart({ dataset, ms, color, editable, initialContainerHeight, store })
      }
